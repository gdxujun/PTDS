...
from .sam2.modeling.sam.double_mask_decoder import MaskDecoder_1, MaskDecoder_2
...
class SAM(nn.Module):
    def __init__(self, inp_size=None, encoder_mode=None, loss=None,
        ...
        if self.fixed_no_obj_ptr:
            assert self.pred_obj_scores
            assert self.use_obj_ptrs_in_encoder
        self.mask_decoder1 = MaskDecoder_1(
    ...
    def forward(self,  gt=None, mode='train'):
        bs = 1
        ...
        if self.use_high_res_features_in_sam:
            ...
            self.features["backbone_fpn"][0] = self.mask_decoder1.conv_s0(
                self.features["backbone_fpn"][0]
            )
            self.features["backbone_fpn"][1] = self.mask_decoder1.conv_s1(
        ...
        # Predict masks
        low_res_masks, iou_predictions,sam_output_tokens,object_score_logits, msk_feat, up_embed= self.mask_decoder1(
        ...
        masks0 = self.postprocess_masks(low_res_masks, self.inp_size, self.inp_size)
        ps_mask = F.interpolate(low_res_masks,size=(int(low_res_masks.shape[-2]/4), int(low_res_masks.shape[-1]/4)),mode='bilinear')
        # gt = self.postprocess_masks(low_res_masks, self.inp_size, self.inp_size)
        # if mode == 'train':
        ...
        low_res_masks2, iou_predictions2,sam_output_tokens2,object_score_logits2, attn1 =  self.mask_decoder2(
            image_embeddings=self._features["image_embed"],
            image_pe=self.get_dense_pe(),
            sparse_prompt_embeddings=sparse_embeddings,
            dense_prompt_embeddings=dense_embeddings,
        ...
        masks = self.postprocess_masks(low_res_masks2, self.inp_size, self.inp_size)
        # masks = torch.stack([
        ...
        #     masks = masks > 0.0
        self.pre_pred_mask = masks0
        self.pred_mask = masks

    def infer(self, input, gt=None, mode='train'):
        bs = 1
    ...
    def backward_G(self):
        """Calculate GAN and L1 loss for the generator"""
        # stage1
        self.loss1 = self.criterionBCE(self.pre_pred_mask, self.gt_mask)
        if self.loss_mode == 'iou':
            self.loss1 += _iou_loss(self.pre_pred_mask, self.gt_mask)
        ...
        if self.loss_mode == 'iou':
            self.loss_G += _iou_loss(self.pred_mask, self.gt_mask)
        weight = 0.6
        self.loss_G = (1-weight)*self.loss1+(weight)*self.loss_G
        self.loss_G.backward()
